...
```bash
mkdir -p stats
echo -e "Sample\tInput_Read_Pairs\tBoth\tBoth_per\tForward_Only\tForward_Only_per\tReverse_Only\tReverse_Only_per\tDropped\tDropped_per" > stats/trimming_stats.txt
paste <(sed '/bash/d ; /TrimmomaticPE/d ; /Using/d ; /phred33/d ; /Input Read Pairs/d ; s/ ILLUMINACLIP.*// ; s/.*\/// ; s/_.*//' jobs/trimming.err.00000000) \
<(grep "Input Read Pairs" jobs/trimming.err.00000000 | sed 's/Input Read Pairs: // ; s/ Both Surviving: /\t/ ; s/ Forward Only Surviving: /\t/ ; s/ Reverse Only Surviving: /\t/ ; s/ Dropped: /\t/ ') | sort >> stats/trimming_stats.txt
```

The resulting file (`stats/trimming_stats.txt`) is used for downstream quality control summaries and reporting.
