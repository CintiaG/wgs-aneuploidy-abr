...
sed '/bash/d ; /TrimmomaticPE/d ; /Using/d ; /phred33/d ; s/.*ppb\/// ; s/\.fq\.gz.*//' jobs/trimming.err.00000000 \
| sed ':a;N;$!ba;s/_2\n/\t/g' \
| sed 's/Input Read Pairs: // ; s/ Both Surviving: /\t/ ; s/ Forward Only Surviving: /\t/ ; s/ Reverse Only Surviving: /\t/ ; s/ Dropped: /\t/ ; s/)//g ; s/ (/\t/g' \
| sort >> out_data/trimming_stats.txt
```
